...
def wraparound(model1, model2, new_images):
    ...
    for i in range(TILE_HEIGHT // step - 1):
        ...
        for x in range(TILE_WIDTH):
            for y in range(0, TILE_HEIGHT):
                ...
                # wrap around
                tile = model2 if new_y < 0  else model1
                new_y %= TILE_HEIGHT
...
for file in inputs:
    ...
    for x_ in range(0, width, TILE_WIDTH):
        for y_ in range(0, height, TILE_HEIGHT):
            tmp = im.crop((x_, y_, x_ + TILE_WIDTH, y_ + TILE_HEIGHT));
            new_images[0].append(tmp)
            if y_ == 0:
                wrap_tile = tmp

            if height > TILE_HEIGHT and y_ != height - TILE_HEIGHT:
                # Iterate over the number of new tiles to create
                for i in range(step, TILE_HEIGHT // step):
                    new_im = im.crop((x_, y_ + i, x_ + TILE_WIDTH, y_ + i + TILE_HEIGHT))
                    # Add new tile to the list of new images
